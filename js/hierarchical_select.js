...
Drupal.HierarchicalSelect.initialize = function(hsid) {
  ...
  if (undefined == drupalSettings.HierarchicalSelect || undefined == drupalSettings.HierarchicalSelect.settings["hs-" + hsid]) {
    return false;
  }

  // If you set drupalSettings.HierarchicalSelect.pretendNoJS to *anything*,
  // and as such, Hierarchical Select won't initialize its Javascript! It
  // will seem as if your browser had Javascript disabled.
  if (undefined != drupalSettings.HierarchicalSelect.pretendNoJS) {
    return false;
  }
  ...
  drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['updatesEnabled'] = true;
  if (undefined == Drupal.HierarchicalSelect.state["hs-" + hsid]) {
    Drupal.HierarchicalSelect.state["hs-" + hsid] = {};
  }

  this.transform(hsid);
  if (drupalSettings.HierarchicalSelect.settings["hs-" + hsid].resizable) {
    this.resizable(hsid);
  }
...
Drupal.HierarchicalSelect.log = function(hsid, messages) {
  ...
  if (drupalSettings.HierarchicalSelect.initialLog == undefined || drupalSettings.HierarchicalSelect.initialLog["hs-" + hsid] == undefined) {
    return;
  }
  else {
  ...
  if (Drupal.HierarchicalSelect.state["hs-" + hsid].log.length == 0) {
    Drupal.HierarchicalSelect.state["hs-" + hsid].log.push(drupalSettings.HierarchicalSelect.initialLog["hs-" + hsid]);
  }
  else {
...
Drupal.HierarchicalSelect.attachBindings = function(hsid) {
  ...
  .unbind('disable-updates').bind('disable-updates', data, function(e) {
    drupalSettings.HierarchicalSelect.settings["hs-" + e.data.hsid]['updatesEnabled'] = false;
  })
  ...
  .find('.hierarchical-select .selects select').unbind().change(function(_hsid) {
    return function() {
      if (drupalSettings.HierarchicalSelect.settings["hs-" + _hsid]['updatesEnabled']) {
...
Drupal.HierarchicalSelect.preUpdateAnimations = function(hsid, updateType, lastUnchanged, callback) {
  switch (updateType) {
    case 'update-hierarchical-select':
      ...
      var animationDelay = drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['animationDelay'];
...
Drupal.HierarchicalSelect.postUpdateAnimations = function(hsid, updateType, lastUnchanged, callback) {
  ...
  switch (updateType) {
    ...
    case 'create-new-item':
      ...
      // created item!
      var cacheId = drupalSettings.HierarchicalSelect.settings["hs-" + hsid].cacheId;
      for (var otherHsid in drupalSettings.HierarchicalSelect.settings) {
        if (drupalSettings.HierarchicalSelect.settings[otherHsid].cacheId == cacheId) {
...
Drupal.HierarchicalSelect.update = function(hsid, updateType, settings) {
  ...
  post.push({ name : 'ajax_page_state[theme]', value : drupalSettings.ajaxPageState.theme });
  post.push({ name : 'ajax_page_state[theme_token]', value : drupalSettings.ajaxPageState.theme_token });
  ...
  switch (updateType) {
    case 'update-hierarchical-select':
      ...
      if ((value == 'none' && drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['renderFlatSelect'] == false)
          || value.match(/^label_\d+$/)
          || (optionClass == 'has-no-children'
             &&
             (
               (drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['renderFlatSelect'] == false
                || $('#hierarchical-select-'+ hsid +'-wrapper .dropbox').length > 0
               )
               &&
               drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['createNewLevels'] == false
             )
           )
         )
      {
  ...
  var url = drupalSettings.HierarchicalSelect.settings["hs-" + hsid].ajax_path;
  ...
  var ajaxOptions = $.extend({}, Drupal.ajax.prototype, {
    ...
    success: function(response, status) {
      ...
      // jQuery ajax object itself, we are allowing Drupal.AjaxCommands.prototype
      // to misserably fail.
      //
      ...
      for (var i in response) {
        if (response[i]['command'] && Drupal.AjaxCommands.prototype[response[i]['command']]) {
          Drupal.AjaxCommands.prototype[response[i]['command']](this, response[i], status, hsid);
        }
      }
  ...
  if (updateType == 'update-hierarchical-select'
      && drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['renderFlatSelect'] == false
      && drupalSettings.HierarchicalSelect.settings["hs-" + hsid]['createNewItems'] == false
      ...
      && Drupal.HierarchicalSelect.cache.status())
  {
    ...
    Drupal.HierarchicalSelect.preUpdateAnimations(hsid, updateType, lastUnchanged, function() {
      ...
      ajaxOptions.data.push({ name : 'ajax_page_state[theme]', value : drupalSettings.ajaxPageState.theme });
      ajaxOptions.data.push({ name : 'ajax_page_state[theme_token]', value : drupalSettings.ajaxPageState.theme_token });
      for (var key in drupalSettings.ajaxPageState.css) {
        ajaxOptions.data.push({ name : 'ajax_page_state[css][' + key + ']', value : 1});
      }
      for (var key in drupalSettings.ajaxPageState.js) {
        ajaxOptions.data.push({ name : 'ajax_page_state[js][' + key + ']', value : 1});
      }

      // Make it work with jquery update
      if (drupalSettings.ajaxPageState.jquery_version) {
        ajaxOptions.data.push({ name : 'ajax_page_state[jquery_version]', value : drupalSettings.ajaxPageState.jquery_version });
      }
...
Drupal.AjaxCommands.prototype.hierarchicalSelectUpdate = function(ajax, response, status, hsid) {
...
Drupal.AjaxCommands.prototype.hierarchicalSelectSettingsUpdate = function(ajax, response, status, hsid) {
  drupalSettings.HierarchicalSelect.settings["hs-" + response.hsid] = response.settings;
};
